...
if (!module.parent) {
  process.on('message', function (event) {
    if (event.language === 'babel') {
      var processor = require('babel-core').transform,
        code = event.data.source;

      try {
        var output = processor(code, { presets: ['es2015', 'stage-3'], plugins: ['transform-strict-mode'] }).code;
        process.send(JSON.stringify({ result: output }));
        process.exit(0);
      } catch (e) {
        console.error(event.language, 'error:: ', e);
        process.exit(0);
      }
